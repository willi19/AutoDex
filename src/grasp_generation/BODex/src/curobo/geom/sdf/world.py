...
class ContactBuffer:
    ...
    pos_buf: torch.Tensor = None 
    dist_buf: torch.Tensor = None 
    normal_buf: torch.Tensor = None 
    debug_posi: torch.Tensor = None 
    debug_normal: torch.Tensor = None 
    grad_pos_buf: torch.Tensor = None 
    grad_dist_buf: torch.Tensor = None 
    grad_normal_buf: torch.Tensor = None 
    ...
    def update_buffer_shape(self, shape: torch.Size, tensor_args: TensorDeviceType):
        if self.shape is not None and self.shape == shape:
            ...
            self.debug_normal[:] = 0
            self.grad_pos_buf[:] = 0
            self.grad_dist_buf[:] = 0
            self.grad_normal_buf[:] = 0
            return self
        batch, horizon, n_spheres, n_perturb = shape
        self.shape = shape
        self.pos_buf = tensor_args.to_device(torch.zeros((batch, horizon, n_spheres, 3)))
        self.dist_buf = tensor_args.to_device(torch.zeros((batch, horizon, n_spheres)))
        self.normal_buf = tensor_args.to_device(torch.zeros((batch, horizon, n_spheres, 3)))
        self.debug_posi = tensor_args.to_device(torch.zeros((batch, horizon, n_spheres, n_perturb, 3)))
        self.debug_normal = tensor_args.to_device(torch.zeros((batch, horizon, n_spheres, n_perturb, 3)))
        self.grad_pos_buf = tensor_args.to_device(torch.zeros((batch, horizon, n_spheres, n_perturb, 3, 4)))
        self.grad_dist_buf = tensor_args.to_device(torch.zeros((batch, horizon, n_spheres, n_perturb, 4)))
        self.grad_normal_buf = tensor_args.to_device(torch.zeros((batch, horizon, n_spheres, n_perturb, 3, 4)))
        return 
...
class WorldCollisionConfig:
    ...
    contact_obj_names: List[str] = None 
    contact_obj_sample_params: Dict = None
    surface_sample_positions: torch.Tensor = None 
    surface_sample_normals: torch.Tensor = None 
    surface_sample_ind_upper: torch.Tensor = None 
    surface_sample_ind_lower: torch.Tensor = None 
    contact_obj_mesh_loading: bool = False
...
class WorldPrimitiveCollision(WorldCollision):
    ...
    def __init__(self, config: WorldCollisionConfig):
        ...
        self._eval_mesh_idx = None 
        if self.world_model is not None:
            if isinstance(self.world_model, list):
                self.load_batch_collision_model(self.world_model)
            else:
                self.load_collision_model(self.world_model)
            if self.contact_obj_names is not None:
                self.load_contact_obj(self.world_model, self.contact_obj_names)
