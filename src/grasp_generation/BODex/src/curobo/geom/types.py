...
class Obstacle:
    ...
    def get_samples_on_surface(self, sample_num, inflate, convex_hull, return_radius):
        m = self.get_trimesh_mesh()
        # apply pose transformation
        transform_matrix = trimesh.scene.transforms.kwargs_to_matrix(translation=self.pose[:3], quaternion=self.pose[3:])
        m.apply_transform(transform_matrix)
        # get inflated convex hull
        m.vertices += inflate * m.vertex_normals
        sample_mesh = m.convex_hull if convex_hull else m
        # sample points
        p, face_index = trimesh.sample.sample_surface_even(sample_mesh, sample_num)
        p, face_index = p.view(np.ndarray), face_index.view(np.ndarray)
        # deal with corner case
        repeat_num = math.ceil(sample_num / p.shape[0])
        p = np.tile(p, (repeat_num, 1))[:sample_num]
    ...
    def get_ray_interaction(self, ray_origin, ray_diection):
        m = self.get_trimesh_mesh()
        center = (m.vertices.max(axis=0) + m.vertices.min(axis=0)) / 2
        ray_origin += center[None]
        p, _, _ = m.convex_hull.ray.intersects_location(ray_origin, ray_diection, multiple_hits=False)
        return p.view(np.ndarray)
...
class Mesh(Obstacle):
    ...
    def get_trimesh_mesh(self, process: bool = True, process_color: bool = True) -> trimesh.Trimesh:
        ...
        if self.vertices is not None and self.faces is not None:
            ...
            self.vertices = m.vertices.view(np.ndarray)
            self.faces, self.vertex_normals = m.faces, m.vertex_normals
            self.vertex_colors, self.face_colors = None, None 
        return m
...
class WorldConfig(Sequence):
    ...
    def __post_init__(self):
        ...
        self.mesh_name_list = [m.name for m in self.mesh]

    def find_mesh_by_name(self, name):
        idx = 0#self.mesh_name_list.index(name)
        return idx, self.mesh[idx]
    ...
    def clone(self) -> WorldConfig:
        ...
        return WorldConfig(
            ...
            mesh=self.mesh.copy() if self.cuboid is not None else None,
