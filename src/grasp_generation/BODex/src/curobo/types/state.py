...
class JointState(State):
    ...
    def reshape(self, dims: torch.Size):
        p = torch.reshape(self.position, dims + (self.position.shape[-1],))
        v = a = j = None
        if self.velocity is not None:
            v = torch.reshape(self.velocity, dims + (self.velocity.shape[-1],))
        if self.acceleration is not None:
            a = torch.reshape(self.acceleration, dims + (self.acceleration.shape[-1],))
        if self.jerk is not None:
            j = torch.reshape(self.jerk, dims + (self.jerk.shape[-1],))
        return JointState(p, v, a, self.joint_names, jerk=j)
