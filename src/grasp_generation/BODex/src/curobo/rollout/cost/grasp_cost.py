...
class GraspCost(CostBase, GraspCostConfig):
    ...
    def evaluate(self, link_pos_quat, env_query_idx):
        ...
        contact_point = ho_contact_point[..., :6]
        grasp_energy, grasp_error, contact_frame, contact_force = self.GraspEnergy.forward(contact_point[..., :3].squeeze(1), contact_normal.squeeze(1), self.TWS['w'])
    ...
    def forward(self, robot_sphere_in, link_pos_quat, env_query_idx, opt_progress):
        ...
        # print(self.contact_query_mode, opt_progress.item(), ge_stage_flag)
