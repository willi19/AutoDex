...
from curobo.rollout.cost.grasp_cost import GraspCost, GraspCostConfig
...
from curobo.util.tensor_util import cat_sum, cat_sum_horizon, cat_max
...
class ArmCostConfig:
    ...
    grasp_cfg: Optional[GraspCostConfig] = None
...
class ArmBase(RolloutBase, ArmBaseConfig):
    ...
    def get_metrics(self, state: Union[JointState, KinematicModelState]):
        ...
        out_metrics.cost, _ = self.cost_fn(state)
        return out_metrics
    ...
    def rollout_fn(self, act_seq, opt_progress=1.0, debug_flag=False) -> Trajectory:
        """
