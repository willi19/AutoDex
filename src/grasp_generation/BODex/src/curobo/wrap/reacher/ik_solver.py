...
class IKSolverConfig:
    ...
    def load_from_robot_config(
        ...
        if contact_strategy is not None:
            config_data["cost"]["contact_strategy"] = contact_strategy
            grad_config_data["cost"]["contact_strategy"] = contact_strategy
            base_config_data["constraint"]["contact_strategy"] = contact_strategy
...
class IKSolver(IKSolverConfig):
    ...
    def _get_result(
        ...
        new_js = JointState.from_position(q_sol, joint_names=self.rollout_fn.kinematics.joint_names)
    ...
    def update_primitive_contact_distance(self, contact_distance: float):
        self.rollout_fn.update_primitive_contact_distance(contact_distance)
        return 
    
    def update_world(self, world: List[WorldConfig]) -> None:
