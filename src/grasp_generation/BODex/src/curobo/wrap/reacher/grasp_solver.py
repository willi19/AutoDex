...
class GraspSolverConfig:
    ...
    def load_from_robot_config(
        ...
        if collision_activation_distance is not None: # None
...
class GraspSolver(GraspSolverConfig):
    ...
    def solve_from_solve_state(
        ...
        # print(solve_state, num_seeds, retract_config, seed_config, return_seeds, use_nn_seed, newton_iters, link_poses, "input s2ss")
        # create goal buffer:
    ...
    def get_result(
        ...
        all_dist = result.metrics.dist_error
...
def get_success(
    ...
    converge = torch.logical_and(
        ...
        dist_error.abs().mean(dim=-1).squeeze(1) <= distance_threshold,
    ).view(-1, num_seeds)
