...
class MotionGenConfig:
    ...
    def load_from_robot_config(
        ...
        base_config_data["world_collision_checker_cfg"]["contact_obj_names"] = contact_obj_names
...
class MotionGen(MotionGenConfig):
    ...
    def update_world(self, world: WorldConfig, contact_obj_names: str = None):
    ...
    def warmup(
        ...
        if batch is None:
            ...
            link_poses = state.link_pose if warmup_link_poses else None 
    ...
    def _plan_from_solve_state_batch(
        ...
        log_warn(f'IK success rate: {ik_result.success.int().sum() / len(ik_result.success)}')
