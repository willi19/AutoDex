...
def upload_results(base_dir):
    ...
    for obj_dir in sorted(cache_base.iterdir()):
        ...
        for idx_dir in sorted(obj_dir.iterdir()):
            ...
            # Upload obj_mask and obj_mask_debug
            for subdir in ["obj_mask", "obj_mask_debug"]:
                ...
                for f in sorted(src_dir.glob("*.avi")):
                    dst = dst_dir / f.name
                    if dst.exists():
                        continue
                    t1 = time.time()
                    ...
                    total_size += sz
                    count += 1
                    print(f"  [{count}] {obj_dir.name}/{idx_dir.name}/{subdir}/{f.name} ({sz/1e6:.1f} MB, {time.time()-t1:.1f}s)", flush=True)
