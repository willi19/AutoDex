"""Batch-render pregrasp-frame grasp+object overlay thumbnails per episode.

Per-episode output: grasp_overlay_thumbnail/{hand}/{obj}/{ep}/
  thumb_{serial}.png × N + thumb_grid.png

Usage:
    python src/process/batch_grasp_thumbnail.py --hand inspire
    python src/process/batch_grasp_thumbnail.py --hand allegro --obj banana
    python src/process/batch_grasp_thumbnail.py --hand inspire --ep 20260405_235218
"""
import argparse
import json
import os
...
OUTPUT_BASE = Path.home() / "shared_data" / "AutoDex" / "grasp_overlay_thumbnail"
LOCAL_CACHE = Path.home() / "cache" / "batch_grasp_thumbnail"
OBJ_BASE = Path.home() / "shared_data" / "AutoDex" / "object" / "paradex"
CANDIDATE_BASE = Path.home() / "shared_data" / "AutoDex" / "candidates"
FPOSE_PY = Path.home() / "miniconda3" / "envs" / "foundationpose" / "bin" / "python"
THUMB_SCRIPT = Path.home() / "AutoDex" / "src" / "visualization" / "overlay_grasp_thumbnail_single.py"
...
def candidate_dir_for(hand, obj, scene_info):
    if not scene_info or len(scene_info) != 3:
        return None
    scene_type, scene_id, grasp_idx = scene_info
    cand = CANDIDATE_BASE / hand / "selected_100" / obj / scene_type / scene_id / grasp_idx
...
def run_thumbnail(local_ep, nas_ep, obj_name, hand, candidate_dir, local_out, frame_pbar=None):
    ...
    cmd = [
        ...
        "--object_mesh", str(OBJ_BASE / obj_name / "raw_mesh" / f"{obj_name}.obj"),
        "--candidate_dir", str(candidate_dir),
        "--result_json", str(nas_ep / "result.json"),
        "--timestamps", str(nas_ep / "raw" / "timestamps" / "timestamp.npy"),
        "--hand", hand,
...
def discover_work(hand, objects, ep_filter):
    ...
    for obj in objects:
        ...
        for ep in sorted(obj_dir.iterdir()):
            ...
            cand = candidate_dir_for(hand, obj, scene_info)
            if cand is None:
                continue
            serials = sorted(p.stem for p in (ep / "videos").glob("*.avi"))
            if serials:
                eps.append((ep.name, serials, cand))
        if eps:
            work.append((obj, eps))
    return work
...
def main():
    ...
    if not work:
        print("No work to do.")
        return
    flat = [(obj, ep, serials, cand) for obj, eps in work for ep, serials, cand in eps]
    ...
    for ei, (obj, ep, serials, cand) in enumerate(flat):
        ...
        ok = run_thumbnail(local_ep, nas_ep, obj, args.hand, cand, local_out, frame_pbar=frame_pbar)
        if ok:
