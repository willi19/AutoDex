...
RESULT="/tmp/sweep_finger.txt"
echo "Variant,Total,Success,Coll,PhysFail,Rate" > "$RESULT"

LIST=$(mktemp); trap "rm -f $LIST" EXIT
echo "$OBJ" > "$LIST"

set_thumb() { sed -i "s|thumb_force_sensor/[0-9]*|thumb_force_sensor/$1|g" "$MANIP_DIR"/*.yml; }
set_finger() {
...
set_thumb 11
for fidx in 10 11 12 13 14; do
    set_finger $fidx
    run "thumb_11_finger_${fidx}"
done

set_finger 14  # restore
echo
echo "=== Summary ==="
cat "$RESULT"
