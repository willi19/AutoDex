...
class ParadexReader:
  def __init__(self, video_dir, downscale=1, shorter_side=None, zfar=np.inf):
    ...
    # Load intrinsics per image
    intrinsics_path = os.path.join(video_dir, "intrinsics.json")
    if not os.path.exists(intrinsics_path):
      intrinsics_path = os.path.join(video_dir, "intrinsics_adjusted.json")
    ...
    for key, value in intrinsics_data.items():
      if "intrinsics" in value:
        ...
        intrinsics = value["intrinsics_adjusted"]
      self.K_table[key] = np.array(intrinsics).reshape(3, 3)
    
    # Load extrinsics per image (world-to-camera transformations)
    extrinsics_path = os.path.join(video_dir, "extrinsics.json")
    if os.path.exists(extrinsics_path):
      with open(extrinsics_path, 'r') as f:
        extrinsics_data = json.load(f)
      
      self.extrinsics_table = {}
      for key, value in extrinsics_data.items():
        # Convert 3x4 to 4x4 (world-to-camera)
        ext_3x4 = np.array(value)
        ext_4x4 = np.vstack([ext_3x4, np.array([0, 0, 0, 1])])
        self.extrinsics_table[key] = ext_4x4
    else:
      self.extrinsics_table = None
      logging.warning(f"Extrinsics file not found: {extrinsics_path}")
    
    # Get image dimensions from first image
    first_img = cv2.imread(self.color_files[0])
    ...
    if self.downscale != 1:
      for key in self.K_table:
        self.K_table[key] = self.K_table[key].copy()
        self.K_table[key][:2] *= self.downscale
  ...
  def get_K(self, i):
    """Get intrinsics for frame i"""
    id_str = self.id_strs[i]
    return self.K_table[id_str]
  
  def get_extrinsic(self, i):
    """Get extrinsics (world-to-camera) for frame i"""
    if self.extrinsics_table is None:
      return None
    id_str = self.id_strs[i]
    return self.extrinsics_table.get(id_str, None)
  ...
  def get_mask(self, i):
    ...
    base_name = os.path.splitext(os.path.basename(self.color_files[i]))[0]
    mask_dir = os.path.join(self.video_dir, "masks")
    # Try jpg first, then png
    mask_file = os.path.join(mask_dir, f"{base_name}.jpg")
  ...
  def get_depth(self, i):
    ...
    depth = cv2.imread(depth_file, -1) / 1e3  # Convert from mm to meters
    if depth is None:
