...
def _resolve_device(device: str) -> str:
    if device.startswith("cuda"):
        if not torch.cuda.is_available():
            raise RuntimeError(
                "CUDA is not available. SAM3 requires a CUDA-capable GPU."
            )
        return device
    raise RuntimeError("SAM3 image model requires CUDA; CPU is not supported.")
...
def generate_masks(data_dir: str, text_prompt: str, device: str, confidence_threshold: float):
    ...
    for image_path in image_paths:
        ...
        print(f"[step1] {base_name}: {elapsed:.3f}s")
    total_elapsed = time.perf_counter() - total_start
    print(f"[step1] total: {total_elapsed:.3f}s for {len(image_paths)} images")


def build_argparser():
    parser = argparse.ArgumentParser(description="Step 1: SAM3 mask generation")
    parser.add_argument(
        "--data-dir",
