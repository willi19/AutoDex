...
from rsslib.conversion import se32cart, cart2se3, se32action
from rsslib.path import candidate_path, obj_path, urdf_path, bodex_path
...
class Renderer(ViserViewer):
    def __init__(self, version, obj_name, scene_type, scene_idx):
        ...
        # Add grid on floor
        self.add_grid(size=6.0, cell_size=0.1, height=0.0)
    ...
    def _load_current_grasp(self):
        ...
        for grasp_name in grasp_list:
            ...
            allegro_path = os.path.join(urdf_path, "allegro_floating.urdf")
            self.add_robot(f"robot_{grasp_name}", allegro_path)
            # self.robot_dict[f"robot_{grasp_name}"].set_visibility(False)
            viz_data = json.load(open(os.path.join(grasp_path, "sim_eval", "viz_data.json"), "r"))
            qpos_traj = np.array(viz_data["qpos_trajectory"])
            
            obj_traj = []
            for tmpi in range(qpos_traj.shape[0]):
                obj_traj.append(self.obj_pose @ cart2se3(qpos_traj[tmpi, -7:]))
                
            obj_traj = np.array(obj_traj)

            hand_traj = qpos_traj[:, 7:-7]
            wrist_se3_traj = []
            for tmpi in range(hand_traj.shape[0]):
                wrist_se3 = self.obj_pose @ cart2se3(qpos_traj[tmpi, :7])
                wrist_se3[:3,:3] = wrist_se3[:3,:3] @ R_delta

                wrist_se3_traj.append(
                    se32action(wrist_se3, hand_traj[tmpi])  # wrist se3
                )

            traj_dict[f"robot_{grasp_name}"] = np.array(wrist_se3_traj)
            obj_traj_dict["target"] = obj_traj
            # traj_dict[f"robot_{grasp_name}"] = grasp_pose
        self.add_traj("asdf", traj_dict, obj_traj_dict)
