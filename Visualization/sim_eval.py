...
from rsslib.path import bodex_path, obj_path

class BODexVisualizer(ViserViewer):
    def __init__(self):
        super().__init__()

        self.bodex_root = bodex_path
    ...
    def _load_current_grasp(self):
        ...
        bodex_info = np.load(os.path.join(grasp_path, "bodex_info.npy"), allow_pickle=True).item()
        ...
        # Show contact points
        if self.show_contact.value and 'contact_point' in bodex_info:
            contact_points = bodex_info['contact_point'][0]
            for cidx in range(contact_points.shape[0]):
                cp = contact_points[cidx].reshape(3, 2)

                self.add_sphere(f"cp_obj_{cidx}", position=cp[:, 0], radius=0.002, color=(0, 1, 0))
                self.add_sphere(f"cp_robot_{cidx}", position=cp[:, 1], radius=0.002, color=(1, 0, 0))
        ...
        # Update metrics
        success = bodex_info.get('success', False)
        grasp_error = bodex_info.get('grasp_error', np.array([0]))
        dist_error = bodex_info.get('dist_error', np.array([0]))
        
        metrics_str = (
            f"Grasp {grasp_dir} ({grasp_idx+1}/{len(self.filtered_grasp_dirs)})\n"
            f"Success: {success}\n"
            f"Grasp Error: {grasp_error.mean():.4f}\n"
            f"Dist Error: {dist_error.mean():.4f}"
        )
        self.metric_text.value = metrics_str
        
        print(f"[BODexVisualizer] Loaded grasp {grasp_dir}: success={success}")
